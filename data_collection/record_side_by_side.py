"""
DUAL CAMERA RECORDING & EMOTION LOGGING SYSTEM

PURPOSE:
Records passenger camera feed with real-time emotion detection and data logging.
Generates timestamped video files + CSV logs for research analysis.

CORE FUNCTIONALITY:
- Camera capture (index 4) at 1920x1080, rotated 180°
- Real-time emotion detection with custom correction rules
- Enhanced drowsiness detection using rolling average EAR
- Auto-generates video files (video DD HH-MM-SS.avi) + emotion logs
- 10-second interval logging for downstream analysis

PROCESSING PIPELINE:
- Crop + enhance using hybrid approach (local + smart_enhance)
- Face detection via analyze_faces_and_draw() from face_detection_passenger
- Custom emotion rules (fear, angry, surprised corrections)
- Visual overlay with timestamps and warning displays

SYSTEM INTEGRATION:
- Standalone data collection tool (not imported by other modules)
- Imports from: face_detection_passenger module
- Generates datasets for: advanced_log_analysis.py processing

USAGE: Primary research data collection script. Press 'q' to stop recording.
"""
...
from face_detection_passenger import analyze_faces_and_draw, get_face_mesh, smart_enhance
# =========== Parameters ===========
EAR_THRESHOLD = 0.25
